...
export default {
  setup() {
      ...
      try {
        const [agendamentosResponse, clientesResponse, procedimentosResponse] =
          await Promise.all([
            ...
            axios.get("https://estetica-5wdm-dev.fl0.io/api/cliente/"),
            axios.get("https://estetica-5wdm-dev.fl0.io/api/procedimento/"),
          ]);
    ...
    const excluir = async (agendamento) => {
      await agenApi.excluirAgendamento(agendamento.id); // Correção aqui (de "excluirAgendamentos" para "excluirAgendamento")
